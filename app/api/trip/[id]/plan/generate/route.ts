...
6. ארוחות: breakfast, lunch, dinner (ובשבת גם seuda_1/2/3) עם שעות הגיוניות + התאמה למדיניות meals (restaurant/self_cooking/mixed). כאשר meals.style=restaurant או mixed **וקיימת מסעדה מתאימה ב-restaurantsCatalog שאמסור לך** — השתמש בה: כתוב את title כשם המסעדה בעברית, וכלול את הכתובת כ-location_name ו-location_address (בדיוק כפי שמופיעים בקטלוג). ללא מצאי מתאים, הצע ארוחה במלון/דירה בלי location.
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Kosher restaurants from the catalog — Claude should prefer these for
  // meal items instead of inventing venue names.
  const restaurantsCatalog = destination
...
Catalog מסעדות כשרות זמינות (העדפה חזקה — אם יש התאמה, השתמש בהן):
${JSON.stringify(restaurantsCatalog, null, 2)}
...
החזר JSON בפורמט הבא בדיוק:
{
  "days": [
    {
      ...
      "items": [
        ...
        { "time": "13:00", "type": "meal", "title": "מסעדת שלום (Shalom)", "duration_min": 75, "location_name": "מסעדת שלום (Shalom)", "location_address": "Hotel Harmonia by Dukley, Budva" },
  ...
  // Helper: match a meal's restaurant reference back to the static catalog
  // so we can fill lat/lng/canonical address even if Claude only wrote the
  // name. Normalized substring match on Hebrew/English names.
  function matchRestaurant(title: string, locName: string | null | undefined) {
    if (!destination) return null;
    const needle = (locName || title || "").toLowerCase().trim();
    if (!needle) return null;
    return (
      destination.restaurants.find((r) => {
  ...
  const enrichedDays = sanitizedDays.map((d) => ({
    ...
    items: d.items.map((it) => {
      // First, for meal items: enrich from restaurants catalog.
      let next = { ...it };
      if (it.type === "meal") {
        const r = matchRestaurant(it.title, it.location_name);
        if (r) {
          next = {
            ...next,
            location_name: next.location_name ?? r.name,
            location_address: next.location_address ?? r.address,
            location_lat: next.location_lat ?? (typeof r.lat === "number" ? r.lat : null),
            location_lng: next.location_lng ?? (typeof r.lng === "number" ? r.lng : null),
          };
        }
      }
      const v = matchVendor(next);
      if (!v) return next;
      return {
        ...next,
        vendor: {
