...
## 📋 Checklist לבדיקה ידנית / Manual Test Checklist

1. פתח טאב "קבצים" בתוך טיול
2. כפתור "העלה קובץ" — disabled עד שכל 3 השדות מלאים
3. בחר קובץ בלבד → שגיאות inline אחרי ניסיון submit עבור תיאור ותמונה
4. הוסף תיאור + thumbnail → כפתור מתאפשר, העלאה מצליחה
5. תצוגת רשימה — thumbnail 64×64, תיאור מתחת לשם הקובץ
6. רשומות ישנות ללא image_url → אייקון fallback לפי קטגוריה

## ❓ שאלות פתוחות / Open Questions
אין / None

## 🔄 המלצה לשלב הבא / Next Stage Recommendation
**שלב 5** (Realtime מלא) או **שלב 4** (מנוע ארוחות חכם) — לבחירת המשתמש.
